...
export function HeroImageUpload({ control }: HeroImageUploadProps) {
  ...
  const handleUpload = async (files: FileList | null, onChange: (value: SuperheroImage[]) => void, currentImages: SuperheroImage[]) => {
    if (!files?.length) return;
    setIsUploading(true);
    try {
      const uploads = await Promise.all(Array.from(files).map(async (file) => uploadImage.mutateAsync(file) as Promise<SuperheroImage>));
            ...
            <FormControl>
              ...
              >
                <input
                  ...
                  onChange={(e) => handleUpload(e.target.files, field.onChange, images)}
                  ...
                  <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3">
                    {images.map((img) => (
                      <div key={img.id} className="relative aspect-video group overflow-hidden rounded-md border bg-background">
                        <img src={getImageUrl(img.imagePath)} alt="Uploaded" className="h-full w-full object-cover" />
                        <button
                          type="button"
                          onClick={() => field.onChange(images.filter((i) => i.id !== img.id))}
